...
export const NewOrderModal: React.FC<Props> = ({
  ...
  const [packageType, setPackageType] = useState<PackageType>(
    prefillData?.packageType || 'Evrak'
  );

  const [packageCount, setPackageCount] = useState<number>(
    prefillData?.packageCount || 1
  );

  const [courierType, setCourierType] = useState<CourierType>(
    prefillData?.courierType || 'Standart Kurye'
  );

  const [urgency, setUrgency] = useState<UrgencyLevel>(
    prefillData?.urgency || 'Normal'
  );

  const [distanceKm, setDistanceKm] = useState<number>(
    prefillData?.distanceKm || 10
  );
  ...
  // Her yeni adres değişikliğine benzersiz hesaplama numarası verir.
  // Böylece eski API cevabı yeni sonucu ezemez.
  const calculationRequestRef = useRef(0);

  // Prefill verisini senkronize et
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Önceki isteği geçersiz hale getir
    ...
    debounceTimerRef.current = setTimeout(async () => {
      try {
        ...
        // Eğer kullanıcı adresleri değiştirdiyse eski sonucu kullanma
  ...
  // ============================================================
  // FİYAT
  // distanceKm değiştiğinde otomatik yeniden hesaplanır.
  // ============================================================
  const {
    finalPrice,
    multiplier,
  ...
  // Hook'lar yukarıda çalıştıktan sonra modalı kapat
  if (!isOpen) {
    return null;
  }
