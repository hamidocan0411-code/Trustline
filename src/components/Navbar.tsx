    ...
    try {
      if (currentUser.role === 'courier') {
        try {
          ...
          console.warn('⚠️ Kurye çıkışında canlı konum kapatılamadı:', error);
        }
      }
      await signOut(auth);
    } catch (error) {
    ...
    >
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-[#CCCCCC]">
          ...
          {onToggleIPhoneMode && (
            <button
              ...
              aria-label={isIPhoneMode ? 'iPhone görünümünü kapat' : 'iPhone görünümünü aç'}
            >
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-[#19191E] border border-[#303036] flex items-center justify-center text-[#CCCCCC] hover:border-red-500/50 hover:text-red-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            aria-label="Çıkış yap"
            title="Çıkış yap"
          >
            {isLoggingOut ? (
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="animate-spin">
