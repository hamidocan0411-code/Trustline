...
export function App() {
  ...
  const applyUserProfile = async (profile: UserProfile) => {
    ...
    if (profile.role === "courier" && courierProfile.employmentStatus === "inactive") {
      console.warn("🚫 Pasif kurye hesabı giriş yapmaya çalıştı:", { uid: profile.id, email: profile.email });
    ...
    try {
      const userNotifications = storage.getNotifications(profile.id);
      setNotifications(Array.isArray(userNotifications) ? userNotifications : []);
    } catch (error) { console.warn("⚠️ Bildirimler yüklenemedi:", error); setNotifications([]); }
    if (profile.role === "customer") setActiveTab("home");
    else if (profile.role === "courier") setActiveTab("courier_panel");
    else if (profile.role === "admin") setActiveTab("admin_panel");
  ...
  useEffect(() => {
    ...
    unsubscribe = subscribeToAuth(async (firebaseUser) => {
      ...
      if (!firebaseUser) {
        ...
        try { storage.setCurrentUser(null); } catch { /* ignore */ }
      ...
      try {
        ...
        if (code === "auth/email-not-verified" || code === "auth/email-verification-required") {
          try { storage.setCurrentUser(null); } catch { /* ignore */ }
          setCurrentUser(null); setNotifications([]); setAppError(null); return;
        }
        if (code === "permission-denied") {
          setCurrentUser(null);
          setAppError("Kullanıcı hesabı bulundu ancak Firestore kullanıcı profiline erişilemiyor. Firebase Firestore Rules kontrol edilmeli.");
          return;
        }
    ...
    const initializeRedirect = async () => {
      try {
        ...
        if (redirectProfile) {
          ...
          await applyUserProfile(redirectProfile);
        }
      } catch (error) {
        if (mountedRef.current) console.error("❌ Google redirect işlemi başarısız:", error);
      } finally {
        if (mountedRef.current) {
          redirectCheckFinished.current = true;
          if (authStateResolved.current && !auth.currentUser && !authResolved.current && !inactiveAccount) {
            setCurrentUser(null); setNotifications([]); setProfileLoading(false); setAuthLoading(false); setAppError(null);
          }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      unsubscribe = storage.subscribe(() => {
        if (!mounted) return;
        try { const nextOrders = storage.getOrders(); setOrders(Array.isArray(nextOrders) ? nextOrders : []); } catch (error) { console.error("Orders listener hatası:", error); }
        try { const nextPricing = storage.getPricing(); if (nextPricing) setPricing(nextPricing); } catch (error) { console.error("Pricing listener hatası:", error); }
        if (currentUser) {
          try { const nextNotifications = storage.getNotifications(currentUser.id); setNotifications(Array.isArray(nextNotifications) ? nextNotifications : []); } catch (error) { console.error("Notification listener hatası:", error); }
  ...
  if (authLoading && !inactiveAccount) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0B0B0D] text-white"><div className="text-center"><div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#D6A84F]"><span className="text-3xl font-black text-[#0B0B0D]">T</span></div><div className="text-sm font-black tracking-[0.2em] text-[#D6A84F]">TRUSTLINE</div><div className="mt-1 text-[10px] tracking-[0.3em] text-[#888888]">EXPRESS</div><p className="mt-4 text-xs text-[#666666]">Güvenli bağlantı kuruluyor...</p></div></div>;
  }
  if (profileLoading && !currentUser && !inactiveAccount) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0B0B0D] text-white"><div className="w-full max-w-sm rounded-3xl border border-[#303036] bg-[#19191E] p-8 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#D6A84F]/15"><span className="text-2xl font-black text-[#D6A84F]">T</span></div><h2 className="mt-5 text-lg font-bold text-white">Hesap hazırlanıyor</h2><p className="mt-2 text-xs text-[#999999]">Hesap bilgileriniz yükleniyor...</p><div className="mx-auto mt-5 h-1.5 w-32 overflow-hidden rounded-full bg-[#303036]"><div className="h-full w-1/2 animate-pulse rounded-full bg-[#D6A84F]" /></div></div></div>;
  }
  if (inactiveAccount) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0B0B0D] px-5 text-white"><div className="w-full max-w-md rounded-3xl border border-[#D6A84F]/30 bg-[#19191E] p-8 text-center shadow-2xl"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-[#D6A84F]/15"><span className="text-2xl font-black text-[#D6A84F]">T</span></div><div className="mt-5 text-sm font-black tracking-[0.2em] text-[#D6A84F]">TRUSTLINE</div><div className="mt-1 text-[10px] tracking-[0.3em] text-[#888888]">EXPRESS</div><h2 className="mt-7 text-xl font-black text-white">Hesabınız pasife alınmıştır</h2><p className="mt-3 text-sm leading-6 text-[#999999]">Kurye hesabınız şu anda aktif değildir.<br />Lütfen yönetici ile iletişime geçin.</p><button type="button" onClick={() => { setInactiveAccount(false); setAppError(null); setCurrentUser(null); setActiveTab("home"); }} className="mt-7 w-full rounded-2xl bg-[#D6A84F] px-6 py-3 text-sm font-black text-[#0B0B0D] transition hover:opacity-90">Giriş Ekranına Dön</button></div></div>;
  }
  if (appError && !currentUser) {
    return <div className="flex min-h-screen items-center justify-center bg-[#0B0B0D] px-5 text-white"><div className="w-full max-w-md rounded-3xl border border-red-500/30 bg-[#19191E] p-8 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-red-500/15 text-red-400">!</div><h2 className="mt-4 text-lg font-bold">Bağlantı Hatası</h2><p className="mt-2 text-sm text-[#999999]">{appError}</p><button type="button" onClick={() => window.location.reload()} className="mt-6 rounded-2xl bg-[#D6A84F] px-6 py-3 text-sm font-black text-[#0B0B0D]">Tekrar Dene</button></div></div>;
  }
  if (!currentUser) {
    return <Suspense fallback={<div className="min-h-screen bg-[#0B0B0D]" />}><AuthScreen onLogin={(profile) => { authResolved.current = true; redirectCheckFinished.current = true; void applyUserProfile(profile); }} /></Suspense>;
  }
  ...
  return <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-[#0B0B0D] text-white"><div className="text-sm font-bold text-[#D6A84F]">TrustLine yükleniyor...</div></div>}>
    <div className={`flex min-h-screen flex-col bg-[#0B0B0D] text-white ${isIPhoneMode ? "items-center justify-center px-2 py-4" : ""}`}>
      <div className={`flex w-full flex-col ${isIPhoneMode ? "h-[820px] max-w-[390px] overflow-hidden rounded-[48px] border-[8px] border-[#222229] bg-[#19191E] shadow-2xl" : "min-h-screen"}`}>
        {isIPhoneMode && <div className="flex h-8 shrink-0 items-center justify-center bg-[#0B0B0D]"><div className="h-5 w-28 rounded-b-xl bg-[#222229]" /></div>}
        <Navbar currentUser={currentUser} unreadNotificationsCount={unreadNotificationsCount} onOpenNotifications={() => setIsNotificationsOpen(true)} isIPhoneMode={isIPhoneMode} onToggleIPhoneMode={() => setIsIPhoneMode((value) => !value)} activeTab={activeTab} />
        <main className={`flex-1 overflow-y-auto px-4 py-5 ${isIPhoneMode ? "max-h-[740px]" : "lg:pl-60"}`}>
          {currentUser.role === "customer" && <>{activeTab === "home" && <><CustomerHome onOpenNewOrder={handleOpenNewOrder} onOpenPharmacyOrder={handleOpenPharmacyOrder} onOpenAI={() => setActiveTab("ai")} onGoToOrders={() => setActiveTab("orders")} activeOrders={activeOrders} pricing={pricing} /><LiveSupport embedded /></>}{activeTab === "orders" && <CustomerOrders orders={myOrders} onOpenNewOrder={handleOpenNewOrder} selectedOrderId={selectedOrderId} />}{activeTab === "ai" && <TrustlineAI onTransferToOrder={handleTransferFromAI} orders={myOrders} pricing={pricing} />}{activeTab === "profile" && <ProfileView currentUser={currentUser} onProfileUpdated={setCurrentUser} />}</>}
          {currentUser.role === "courier" && <>{activeTab === "courier_panel" && <CourierPanel currentCourier={currentUser} orders={safeOrders} />}{activeTab === "profile" && <ProfileView currentUser={currentUser} onProfileUpdated={setCurrentUser} />}</>}
          {currentUser.role === "admin" && <>{activeTab === "admin_panel" && <AdminPanel orders={safeOrders} pricing={pricing} />}{activeTab === "profile" && <ProfileView currentUser={currentUser} onProfileUpdated={setCurrentUser} />}</>}
        </main>
        <BottomNavigation role={currentUser.role} activeTab={activeTab} onTabChange={(tab) => { setActiveTab(tab); setSelectedOrderId(null); }} onOpenNewOrder={() => handleOpenNewOrder()} activeOrdersCount={activeOrders.length} isIPhoneMode={isIPhoneMode} />
      </div>
      <NewOrderModal isOpen={isNewOrderOpen} onClose={() => { setIsNewOrderOpen(false); setNewOrderPrefill(undefined); }} currentUser={currentUser} pricing={pricing} prefillData={newOrderPrefill} onOrderCreated={(order) => { setActiveTab("orders"); setSelectedOrderId(order.id); }} />
      <PharmacyOrderPanel isOpen={isPharmacyOrderOpen} onClose={() => setIsPharmacyOrderOpen(false)} currentUser={currentUser} pricing={pricing} onOrderCreated={(order) => { setActiveTab("orders"); setSelectedOrderId(order.id); }} />
      <NotificationDrawer isOpen={isNotificationsOpen} onClose={() => setIsNotificationsOpen(false)} notifications={notifications} userId={currentUser.id} onSelectOrder={(orderId) => { setSelectedOrderId(orderId); setActiveTab(currentUser.role === "courier" ? "courier_panel" : "orders"); }} />
    </div>
  </Suspense>;
}
