...
type PublicInfoPageKey = typeof PUBLIC_INFO_PAGES[keyof typeof PUBLIC_INFO_PAGES];
...
export function App() {
  ...
  const applyUserProfile = async (profile: UserProfile) => {
    ...
    if (profile.role === "customer") setActiveTab("home"); else if (profile.role === "corporate") setActiveTab("corporate_panel"); else if (profile.role === "courier") setActiveTab("courier_panel"); else if (profile.role === "admin") setActiveTab("admin_panel");
  ...
  {activeTab === "corporate_support" && (
    <div className="mx-auto w-full max-w-4xl pb-24">
      <div className="mb-5">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#D6A84F]">Destek Merkezi</p>
        <h1 className="mt-1 text-2xl font-black text-white">Kurumsal Destek</h1>
        <p className="mt-2 text-sm text-[#777780]">Sipariş, teslimat veya hesap işlemleriniz için mevcut destek sistemini kullanın.</p>
      </div>
